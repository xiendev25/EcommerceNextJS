        ...
        const fetchData = async () => {
            ...
            try {
                const [postRes, topicsRes] = await Promise.all([
                    fetch(`${API_BASE}/getPostbyId/${postId}`),
                    fetch(`${API_BASE}/topic`),
                ]);

                if (!postRes.ok) {
                    if (postRes.status === 404) throw new Error(`Không tìm thấy bài viết với ID ${postId}.`);
                    throw new Error(`Lỗi tải bài viết: HTTP ${postRes.status}`);
                }
                ...
                setInitialThumbnailUrl(postJson.image || null);
                    ...
                    <div className="bg-white p-6 shadow-sm rounded-lg border">
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-2">
                                Tải ảnh lên (để trống nếu giữ nguyên)
                            </label>
                            <input
                                type="file"
                                accept="image/*"
                            ...
                            {(thumbnailPreview || initialThumbnailUrl) && (
                                <Image
                                    src={thumbnailPreview || initialThumbnailUrl || ''}
