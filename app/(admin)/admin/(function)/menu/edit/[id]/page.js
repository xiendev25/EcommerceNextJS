...
export default function Page() {
    ...
    useEffect(() => {
        ...
        const fetchMenuData = async () => {
            ...
            try {
                const [menuRes, listRes] = await Promise.all([
                ...
                if (!listRes.ok) throw new Error(`Menu List HTTP ${listRes.status}`);

                const json = await menuRes.json();
                const listJson = await listRes.json();

                setMenuListData(Array.isArray(listJson) ? listJson : []);
                ...
                console.error("Lỗi fetch data menu:", e);
                setState(s => ({ ...s, loading: false, error: e.message || 'Fetch error' }));
            }
        };
        fetchMenuData();
    ...
    const siblings = useMemo(() => {
        return menuListData
            .filter(m => Number(m.parent_id) === Number(form.parent_id) && String(m.type) === String(form.type) && m.id !== Number(id))
            .sort((a, b) => (Number(a.sort_order ?? 0)) - (Number(b.sort_order ?? 0)))
    }, [menuListData, form.parent_id, form.type, id])
    ...
    const handleFormChange = (e) => {
        ...
        if (name === 'parent_id' || name === 'type') {
            ...
            const newSiblings = menuListData.filter(m => Number(m.parent_id) === newParentId && String(m.type) === newType && m.id !== Number(id));
    ...
    if (state.loading) return <div className="p-6 text-sm text-gray-600 animate-pulse">Đang tải dữ liệu menu...</div>;
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-3">
                    ...
                    <div>
                        ...
                        >
                            ...
                            {menuListData
                                .filter(menu => Number(menu.parent_id) === 0 && String(menu.type) === String(form.type) && menu.id !== Number(id))
                                .sort((a, b) => (Number(a.sort_order ?? 0)) - (Number(b.sort_order ?? 0)))
                                .map(menu => (
