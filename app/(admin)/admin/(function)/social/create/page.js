...
export default function Page() {
    ...
    // Removed loading state, use authLoading and submitting state instead
    ...
    // Removed useEffect fetching user (now handled by authLoading)

    // Fetch social icon data
    useEffect(() => {
        if (authLoading) return; // Wait for auth check

        const fetchData = async () => {
            // No need for separate loading state here
            try {
                const res = await fetch(`${API_BASE}/social-icon`);
                if (!res.ok) throw new Error("Không thể tải danh sách các loại mạng xã hội.");

                const json = await res.json();
                const icons = Array.isArray(json) ? json : [];
                setAllSocialIcons(icons);
                // Auto-select first icon if available and none selected
                if (icons.length > 0 && !form.social_id) {
                    setForm(prev => ({ ...prev, social_id: String(icons[0].id) })); // Ensure string
                }
            } catch (e) {
                ...
                <div>
                    ...
                    <div className="flex items-center">
                        ...
                        <input
                            ...
                            required
                            className={`w-full h-12 px-4 border border-gray-300 ${selectedSocial?.link ? 'rounded-r-lg' : 'rounded-lg'}`}
